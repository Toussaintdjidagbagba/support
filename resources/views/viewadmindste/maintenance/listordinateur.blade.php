								...
								<table id="tech-companies-1" class="table table-small-font table-bordered table-striped">
									...
									<tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{App\Providers\InterfaceServiceProvider::getUserOutil($maint->outil)}}</td>
                                            <td>{{ $maint->commentaireuser }}</td>
                                            <td>{{ $maint->avisuser }}</td>
                                            <td>{{App\Providers\InterfaceServiceProvider::LibelleUser($maint->action)}}</td>
                                            <td>{{ $maint->etat }}</td>
                                            <td>{{ $maint->commentaireinf }}</td>
                                            <td>
                                                
                                            @if(in_array("print_maint_admin_pdf", session("auto_action")))
                                            <a href="{{ route('GEMP', $maint->id) }}" onClick="" type="button" title="PDF"  class="btn btn-primary btn-circle btn-xs  margin-bottom-10 waves-effect waves-light">
                                            ...
                                            <button type="button" title="Détails"  class="btn btn-primary btn-circle btn-xs  margin-bottom-10 waves-effect waves-light" data-toggle="modal" data-target="#detail" onclick="setdetailmaintenance('{{$maint->detailjson}}')">
                                            ...
                                            data-toggle="modal" data-target="#update" onclick="setupdatemaintenance({{$maint->id}}, '{{$maint->detailjson}}', '{{App\Providers\InterfaceServiceProvider::periodeMaintenance($maint->maintenance)}}', '{{App\Providers\InterfaceServiceProvider::getLibOutil($maint->outil)}}', '{{ $maint->etat }}', '{{ $maint->commentaireinf }}')">
                                            ...
                                            data-toggle="modal" data-target="#deletemaintenance" onclick="setdeletemaintenance({{$maint->id}}, '{{App\Providers\InterfaceServiceProvider::periodeMaintenance($maint->maintenance)}}', '{{App\Providers\InterfaceServiceProvider::getLibOutil($maint->outil)}}')">
                                        ...
                                        @empty
                                        <tr>
                                            <td colspan="9"><center>Pas de maintenance enregistrer!!!</center> </td>
                                        </tr>
                                        @endforelse
                                    </tbody>
								</table>
        ...
        <script type="text/javascript">
            ...
            async function validemaintenance(){
                ...
                if(periode == 0 || ordinateur == 0)
                    if (periode == 0) {
                        document.getElementById('infomaintenance').innerHTML += "<div class='alert alert-danger alert-block'>Veuillez sélectionner la période de maintenance.. </div>";
                    }
                    if (ordinateur == 0) {
                        document.getElementById('infomaintenance').innerHTML += "<div class='alert alert-danger alert-block'>Veuillez sélectionner l'ordinateur pour lequel la maintenance sera effectuée.. </div>";
                    }
                else{
                    dat = {
                        _token: token, periode: periode, ordinateur: ordinateur, etat: etat, maint: maint, obs: obs
                    };

                    document.getElementById("infomaintenance").innerHTML = '<div class="alert alert-warning alert-block"><button type="button" class="close" data-dismiss="alert">×</button><strong>En cours de traitement.. <br> Veuillez patienter! </strong></div>';
                    ...
                    // En cours d'envoie
                        try {
                            ...
                            if(response.status == 200)
                            {
                                ...
                                document.getElementById("infomaintenance").innerHTML = '<div class="alert alert-success alert-block"><button type="button" class="close" data-dismiss="alert">×</button><strong>'+data+'</strong></div>';
                                setTimeout(function(){
                                    window.location.reload();
                                }, 3000);
                            }
                            else{
                                document.getElementById("infomaintenance").innerHTML = '<div class="alert alert-danger alert-block"> <button type="button" class="close" data-dismiss="alert">×</button><strong>Une erreur s\'est produite </strong></div>';
                            }
                        } catch (error) {
            ...
            async function setdeletemaintenance(id, periode, outil){
                document.getElementById('infodelete').innerHTML = "Vous voulez vraiment supprimer "+outil+" de la période "+periode+" ?";
            ...
            async function setupdatemaintenance(id, maint, periode, outil, etat, commentaire) {
                ...
                document.getElementById('uetat').innerHTML = '<select type="text" id="uuetat" name="etat" class="form-control"> <option> '+etat+' </option> <option> Excellent </option><option> Bien </option><option> Passable </option> <option> Médiocre </option> </select>';
            }
            ...
            async function valideupdatemaintenance(){
                ...
                dat = {_token: token, id: id, etat: etat, maint: maint, obs: obs};
                    ...
                    // En cours d'envoie
                        try {
                            ...
                            {
                                data = await response.text();
                                document.getElementById("infoupdate").innerHTML = '<div class="alert alert-success alert-block"><button type="button" class="close" data-dismiss="alert">×</button><strong>'+data+'</strong></div>';
                                setTimeout(function(){
                                    window.location.reload();
                                }, 3000);
                            }
                            else{
                                document.getElementById("infoupdate").innerHTML = '<div class="alert alert-danger alert-block"> <button type="button" class="close" data-dismiss="alert">×</button><strong>Une erreur s\'est produite </strong></div>';
                            }
                        } catch (error) {
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
                           ...
                           <div class="form-group">
                            <div class="form-line">
                                ...
                                <select type="text" id="periode" name="periode" class="form-control">
                                    <option value="{{ $periode }}">{{ App\Providers\InterfaceServiceProvider::periodeMaintenance($periode) }}</option>
                                    <!--@foreach($allperiode as $periode)
                                        <option value="{{ $periode->id}}"> {{ $periode->periodedebut}} au {{ $periode->periodefin}} </option>
                                    @endforeach-->
                                </select>
                            </div>
                           </div>
                            ...
                            <div class="form-line">
                                ...
                                <select type="text" id="ordinateur" name="ordinateur" class="form-control">
                                    ...
                                    @foreach($allordinateur as $ordinateur)
                                        <option value="{{ $ordinateur->id}}"> {{ $ordinateur->nameoutils}} </option>
                                    @endforeach
                                </select>
                            </div>
                           </div>
                    ...
                    <div class="row clearfix">
                        <div class="col-md-6">
                            <input type="checkbox" id="sft" name="maint" value="sft" class="filled-in chk-col-brown" />
                            <label for="sft">Suppression des fichiers temporaire</label> <br>
                            <input type="checkbox" id="mjw" name="maint" value="mjw" class="filled-in chk-col-brown" />
                            <label for="mjw">Mise à jour Windows</label> <br>
                            <input type="checkbox" id="dfg" name="maint" value="dfg" class="filled-in chk-col-brown" />
                            <label for="dfg">Défragmentation</label> <br>
                            <input type="checkbox" id="rdd" name="maint" value="rdd" class="filled-in chk-col-brown" />
                            <label for="rdd">Réparation des disques</label> <br>
                            <input type="checkbox" id="edd" name="maint" value="edd" class="filled-in chk-col-brown" />
                            <label for="edd">Etat de disque</label> <br>
                            <input type="checkbox" id="epdd" name="maint" value="epdd" class="filled-in chk-col-brown" />
                            <label for="epdd">Espace de disque</label> <br>
                            <input type="checkbox" id="atv" value="atv" name="maint" class="filled-in chk-col-brown" />
                            <label for="atv">Antivirus</label> <br>
                        </div>
                        <div class="col-md-6">
                            <input type="checkbox" id="duc" name="maint" value="duc" class="filled-in chk-col-brown" />
                            <label for="duc">Dépoussièrer Unité Centrale</label> <br>
                            <input type="checkbox" id="dram" name="maint" value="dram" class="filled-in chk-col-brown" />
                            <label for="dram">Dépoussièrer RAM</label> <br>
                            <input type="checkbox" id="dcs" value="dcs" name="maint" class="filled-in chk-col-brown" />
                            <label for="dcs">Dépoussièrer Clavier/Souris</label> <br>
                            <input type="checkbox" id="decr" value="decr" name="maint" class="filled-in chk-col-brown" />
                            <label for="decr">Dépoussièrer Ecran</label> <br>
                            <input type="checkbox" id="bkpi" value="bkpi" name="maint" class="filled-in chk-col-brown" />
                            <label for="bkpi">Backup interne </label> <br>
                            <input type="checkbox" id="bkpe" value="bkpe" name="maint" class="filled-in chk-col-brown" />
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    <div class="row clearfix">
                        <div class="col-md-6">
                            <input type="checkbox" id="dsft" name="maint" value="dsft"  class="filled-in chk-col-brown" />
                            <label for="dsft">Suppression des fichiers temporaire</label> <br>
                            <input type="checkbox" id="dmjw" name="maint" value="dmjw" class="filled-in chk-col-brown" />
                            <label for="dmjw">Mise à jour Windows</label> <br>
                            <input type="checkbox" id="ddfg" name="maint" value="ddfg" class="filled-in chk-col-brown" />
                            <label for="ddfg">Défragmentation</label> <br>
                            <input type="checkbox" id="drdd" name="maint" value="drdd" class="filled-in chk-col-brown" />
                            <label for="drdd">Réparation des disques</label> <br>
                            <input type="checkbox" id="dedd" name="maint" value="dedd" class="filled-in chk-col-brown" />
                            <label for="dedd">Etat de disque</label> <br>
                            <input type="checkbox" id="depdd" name="maint" value="depdd" class="filled-in chk-col-brown" />
                            <label for="depdd">Espace de disque</label> <br>
                            <input type="checkbox" id="datv" value="datv" name="maint" class="filled-in chk-col-brown" />
                            <label for="datv">Antivirus</label> <br>
                        </div>
                        <div class="col-md-6">
                            <input type="checkbox" id="dduc" name="maint" value="dduc" class="filled-in chk-col-brown" />
                            <label for="dduc">Dépoussièrer Unité Centrale</label> <br>
                            <input type="checkbox" id="ddram" name="maint" value="ddram" class="filled-in chk-col-brown" />
                            <label for="ddram">Dépoussièrer RAM</label> <br>
                            <input type="checkbox" id="ddcs" value="ddcs" name="maint" class="filled-in chk-col-brown" />
                            <label for="ddcs">Dépoussièrer Clavier/Souris</label> <br>
                            <input type="checkbox" id="ddecr" value="ddecr" name="maint" class="filled-in chk-col-brown" />
                            <label for="ddecr">Dépoussièrer Ecran</label> <br>
                            <input type="checkbox" id="dbkpi" value="dbkpi" name="maint" class="filled-in chk-col-brown" />
                            <label for="dbkpi">Backup interne </label> <br>
                            <input type="checkbox" id="dbkpe" value="dbkpe" name="maint" class="filled-in chk-col-brown" />
            ...
            <div class="modal-body">
                    ...
                    <div class="row clearfix">
                        ...
                        <div class="col-md-6">
                           ...
                           <div class="form-group">
                            <div class="form-line">
                                <input type="text" id="uobs" name="obs" class="form-control" placeholder="">
                            </div>
                           </div>
                        </div>
                    </div>
                    ...
                    <div class="row clearfix">
                        <div class="col-md-6">
                            <input type="checkbox" id="usft" name="umaint" value="sft" class="filled-in chk-col-brown" />
                            <label for="usft">Suppression des fichiers temporaire</label> <br>
                            <input type="checkbox" id="umjw" name="umaint" value="mjw" class="filled-in chk-col-brown" />
                            <label for="umjw">Mise à jour Windows</label> <br>
                            <input type="checkbox" id="udfg" name="umaint" value="dfg" class="filled-in chk-col-brown" />
                            <label for="udfg">Défragmentation</label> <br>
                            <input type="checkbox" id="urdd" name="umaint" value="rdd" class="filled-in chk-col-brown" />
                            <label for="urdd">Réparation des disques</label> <br>
                            <input type="checkbox" id="uedd" name="umaint" value="edd" class="filled-in chk-col-brown" />
                            <label for="uedd">Etat de disque</label> <br>
                            <input type="checkbox" id="uepdd" name="umaint" value="epdd" class="filled-in chk-col-brown" />
                            <label for="uepdd">Espace de disque</label> <br>
                            <input type="checkbox" id="uatv" value="atv" name="umaint" class="filled-in chk-col-brown" />
                            <label for="uatv">Antivirus</label> <br>
                        </div>
                        <div class="col-md-6">
                            <input type="checkbox" id="uduc" name="umaint" value="duc" class="filled-in chk-col-brown" />
                            <label for="uduc">Dépoussièrer Unité Centrale</label> <br>
                            <input type="checkbox" id="udram" name="umaint" value="dram" class="filled-in chk-col-brown" />
                            <label for="udram">Dépoussièrer RAM</label> <br>
                            <input type="checkbox" id="udcs" value="dcs" name="umaint" class="filled-in chk-col-brown" />
                            <label for="udcs">Dépoussièrer Clavier/Souris</label> <br>
                            <input type="checkbox" id="udecr" value="decr" name="umaint" class="filled-in chk-col-brown" />
                            <label for="udecr">Dépoussièrer Ecran</label> <br>
                            <input type="checkbox" id="ubkpi" value="bkpi" name="umaint" class="filled-in chk-col-brown" />
                            <label for="ubkpi">Backup interne </label> <br>
                            <input type="checkbox" id="ubkpe" value="bkpe" name="umaint" class="filled-in chk-col-brown" />
