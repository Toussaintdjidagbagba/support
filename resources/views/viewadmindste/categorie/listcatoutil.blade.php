	...
	<div class="container-fluid">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            ...
                            <button type="button" style="margin-right: 30px; float: right; padding-right: 30px; padding-left: 30px;" class="btn bg-deep-orange waves-effect" data-toggle="modal" data-target="#add">Ajouter</button>
                       		</h2>
                        </div>
                        <div class="body">
								...
								<table id="tech-companies-1" class="table table-small-font table-bordered table-striped">
									...
									<tbody>
                                        ...
                                        <tr>
                                            <td>
                                                {{$cat->libelle}}
                                                <p style="text-align: right; font-size: 11px; color: #111111">Enregistré par {{App\Providers\InterfaceServiceProvider::LibelleUser($cat->action)}}</p>
                                            </td>
                                            <td>
                                            @if(in_array("update_cat_outil", session("auto_action")))
                                            <button type="button" title="Modifier" onclick="avisupdate({{$cat->id}}, '{{$cat->libelle}}')" class="btn btn-primary btn-circle btn-xs  margin-bottom-10 waves-effect waves-light" data-toggle="modal" data-target="#update">
                                                <i class="material-icons">system_update_alt</i>
                                            </button>
                                            @endif

                                            @if(in_array("delete_cat_outil", session("auto_action")))
                                            <button type="button" title="Supprimer" onclick="avissuppression({{$cat->id}}, '{{$cat->libelle}}')" class="btn btn-danger btn-circle btn-xs  margin-bottom-10 waves-effect waves-light" 
                                            data-toggle="modal" data-target="#del"  style="color:white;"><i class="material-icons">delete_sweep</i></button>
                                            @endif

                                            @if(in_array("define_champ_cat_outil", session("auto_action")))
                                            <button type="button" title="Définition" onclick="avischamp({{$cat->id}}, '{{$cat->libelle}}')" class="btn btn-primary btn-circle btn-xs  margin-bottom-10 waves-effect waves-light" data-toggle="modal" data-target="#carac">
        ...
        <script type="text/javascript">
            ...
            async function confirmesuppression(){
                ...
                document.getElementById("infosdel").innerHTML = '<div class="alert alert-warning alert-block"><button type="button" class="close" data-dismiss="alert">×</button><strong>En cours de traitement.. <br> Veuillez patienter! </strong></div>';
                        ...
                        let response = await fetch("{{route('DCO')}}?_token="+token+"&id="+iddel, 
                        {
                            ...
                            headers: {
                                    ...
                                    if(response.status == 200)
                                    {
                                        ...
                                        document.getElementById("infosdel").innerHTML = '<div class="alert alert-success alert-block"><button type="button" class="close" data-dismiss="alert">×</button><strong>'+data+'</strong></div>';
                                        setTimeout(function(){
                                        ...
                                        document.getElementById("infosdel").innerHTML = '<div class="alert alert-danger alert-block"><button type="button" class="close" data-dismiss="alert">×</button><strong>Une erreur s\'est produite </strong></div>';
                                    }
                    } catch (error) {
            ...
            async function confirmechamp(){
                ...
                document.getElementById("infocarac").innerHTML = '<div class="alert alert-warning alert-block"><button type="button" class="close" data-dismiss="alert">×</button><strong>En cours de traitement.. <br> Veuillez patienter! </strong></div>';
                    ...
                    try {
                        let response = await fetch("{{route('SCCLO')}}",
                        {
                            ...
                            body: JSON.stringify(dat)
                                    ...
                                    {
                                        data = await response.text();
                                        document.getElementById("infocarac").innerHTML = '<div class="alert alert-success alert-block"><button type="button" class="close" data-dismiss="alert">×</button><strong>'+data+'</strong></div>';
                                        document.getElementById("caraclib").value = "";
                                        getchampincategorie(id);
                                    }
                                    else{
                                        document.getElementById("infocarac").innerHTML = '<div class="alert alert-danger alert-block"><button type="button" class="close" data-dismiss="alert">×</button><strong>Une erreur s\'est produite </strong></div>';
                                    }
                    } catch (error) {
                        console.log(error);
                ...
                try {
                        ...
                        if(response.status == 200)
                        {
                            ...
                            list.forEach(function(currentline, index, arry){
                                contenu += '<label style="float:left; margin:10px; padding:10px; background:#c1cdcd; border-radius :10%; color:black;">';
...
<div class="modal fade" id="del" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default btn-sm waves-effect waves-light" data-dismiss="modal">FERMER</button>
                <button onclick="confirmesuppression()" class="btn btn-danger waves-effect waves-light">SUPPRIMER</button>
            </div>
        </div>
    </div>
</div>
            ...
            <div class="modal-body">
                    ...
                    <div class="row clearfix">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                            <div class="form-line">
                                <input type="text" id="infolib" name="infolib" class="form-control" placeholder="">
                            </div>
                           </div>
                        </div>
                    </div>
...
<div class="modal fade" id="carac" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">Définitions des champs caractéristiques de la catégorie <i id="catcarac"> </i> : </h4>
            </div>
            <div class="modal-body">
                    <input type="hidden" id="_token" value="{{ csrf_token() }}" />
