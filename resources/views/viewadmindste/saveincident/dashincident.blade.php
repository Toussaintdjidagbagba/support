	...
	<div class="container-fluid">
								...
								<table id="tech-companies-1" class="table table-small-font table-bordered table-striped">
									...
									<tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{date("h:i:s", strtotime("-1 hours ".date("h:i:s", strtotime ( "+".App\Providers\InterfaceServiceProvider::TempsCat($inc->cat)." hours ".date('h:i:s', strtotime($inc->created_at)) ) - strtotime (date('h:i:s'))))) }}</td>
                                            <td>{{$inc->etat}}</td>
                                            <td><span class="co-name">
                                                @if( $inc->avis == "" ||  $inc->avis == null)
                                                <a class="btn bg-blue btn-circle waves-effect waves-circle waves-float" 
                                                onclick="getid({{ $inc->id }})" data-id="getid({{ $inc->id }})"
                                                data-color="deep-orange" data-toggle="modal" 
                                                data-target="#avis"><i class="material-icons">grade</i></a>
                                                @else
                       		                    {{ $inc->avis }}
                       		                    @endif
                                                </span>
                                            ...
                                            <td>
                                                 @if( $inc->statut != 1)
                                                    ...
                                                    <button type="button" title="Modifier"  class="btn btn-primary btn-circle btn-xs  margin-bottom-10 waves-effect waves-light">
                                                        <a href="{{ route('MTI', $inc->id)}}" style="color:white;"> <i class="material-icons">system_update_alt</i></a> 
                                                        
                                                    </button>
                                                    @endif

                                                    @if(in_array("delete_incident", session("auto_action")))
                                                    <button type="button" title="Supprimer"  class="btn btn-danger btn-circle btn-xs  margin-bottom-10 waves-effect waves-light"><a href="{{ route('DI', $inc->id) }}" style="color:white;"><i class="material-icons">delete_sweep</i></a> </button>
                                                    @endif
                                                @endif
                                            </td>
                                        </tr>
                                        @empty
                                        <tr>
                                            <td colspan="9"><center>Pas d'incident enregistrer!!!</center> </td>
                                        </tr>
                                        @endforelse
                                    </tbody>
								</table>
        ...
        <script>
            ...
            async function validavis(){
        		...
        		try {
                	let response = await fetch("{{route('GIAvis')}}?_token="+token+"&anormalieid="+anormalieid+"&avis="+lib+"&libsub="+libsub, 
...
<div class="modal fade" id="avis" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title" id="myModalLabel">Comment avez-vous trouvé le traitement de votre préoccupation ? </h4>
			</div>
            <div class="modal-body">
