                    ...
                    <div class="card">
                        <div class="header">
                            ...
                            <button type="button" style="margin-right: 30px; float: right; padding-right: 30px; padding-left: 30px;" 
                                class="btn bg-deep-orange waves-effect" data-color="deep-orange"> <a href="{{ route('GI') }}" style="text-style:none; color:white">Incidents</a>
                            </button> 
                                <div class="row clearfix">
                                    <div class="col-md-2">
 ...
 <script>
    ...
    try {
        ...
        let response = await fetch("{{ route('getstatistique') }}",
        {
            ...
            body: JSON.stringify({ periodedebut: 0, periodefin: 0 })
        }); 
        ...
        if (response.status == 200) {
            ...
            depenseslist.forEach(enc => {
                ...
                newRow += '<td>' + enc.date + '</td>'; */
                    newRow += '<tr>';
                    newRow += '<td>' + enc.description + '</td>';
                    newRow += '<td style="text-align: right">' + new Intl.NumberFormat('fr-FR').format( Math.round(enc.montant)) + '</td>';
                    newRow += '<td style="text-align: right">' + new Intl.NumberFormat('fr-FR').format( Math.round(enc.montantht)) + '</td>';
                    newRow += '<td style="text-align: right">' + new Intl.NumberFormat('fr-FR').format( Math.round(enc.montanttva) ) + '</td>';
                    newRow += '<td>' + enc.numpiece + '</td>';
                    newRow += '<td>' + enc.datefacture + '</td>';
                    newRow += '<td>' + enc.dateexigibilite + '</td>';
                    newRow += '<td> Non réglé </td>';
                    newRow += `<td> <button type="button" title="Exécuter"  class="btn btn-primary btn-circle btn-xs  margin-bottom-10 waves-effect waves-light" data-toggle="modal" data-target="#exec" data-values='["${enc.id}","${escapeQuotes(enc.description)}", "${enc.montantht}" , "${enc.montanttva}", "${enc.montant}"]'  onclick="getexecute(this)"><i class="material-icons">play_circle_filled</i></a> </button> </td>`;
            ...
            if(newRow == ""){
                document.getElementById('listdepens').innerHTML = '<tr> <td colspan="9"><center> Aucun décaissement réalisé disponible pour ce projet. </center> </td> </tr>';
            }else{
