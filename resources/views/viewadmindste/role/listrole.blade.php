                                ...
                                <tbody>
                                    @forelse($list as $role)
                                        <tr>
                                            ...
                                            <td>{{ $role->libelle }}</td>
                                            <td>{{ App\Providers\InterfaceServiceProvider::LibelleUser($role->user_action) }}
                                            </td>
                                            ...
                                            <td>
                                                @if (in_array('update_role', session('auto_action')))
                                                    <button type="button" title="Modifier"
                                                        ...
                                                        <a href="{{ url('modif-roles-') }}{{ $role->idRole }}"
                                                            style="color:white;"> <i
                                                ...
                                                @if (in_array('delete_role', session('auto_action')))
                                                    <button type="button" title="Supprimer" style="color:white;"
                                                        data-token="{{ csrf_token() }}" data-Id="{{ $role->idRole }}"
                                                        onclick="Delete(event, '{{ route('DR') }}','{{ $role->libelle }}')"
                                                ...
                                                @if (in_array('menu_role', session('auto_action')))
                                                    <button type="button" title="Menu"
                                                        class="btn btn-warning btn-circle btn-xs  margin-bottom-10 waves-effect waves-light"><a
                                                            href="{{ url('menu-roles-') }}{{ $role->idRole }}"
                                                            style="color:white;"> <i
                                                                class="material-icons green-color">menu_book</i></a></button>
                                                @endif
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="7">
                                                <center>Pas de r�0�0le enregistrer!!!</center>
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
    ...
    <script type="text/javascript">
        async function Delete(event, url, libelle) {
            ...
            const {
                ...
                title: 'Êtes-vous sûr de vouloir supprimer le rôle <span class="text-danger">' + libelle +
                    '</span> ?',
