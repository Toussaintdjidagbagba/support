								...
								<table id="tech-companies-1" class="table table-small-font table-bordered table-striped">
									...
									<tbody>
                                        ...
                                        <tr>
                                            <td>
                                                @if($user->image != "")
                                                    <img src="{{ asset('images/' . $user->image) }}" alt="Image de profil" width="45" height="55" style="border-radius: 50%">
                                                @else
                                            ...
                                            <button type="button" onclick="updateuser({{$user->idUser}})" title="Modifier"  class="btn btn-primary btn-circle btn-xs  margin-bottom-5 waves-effect waves-light">
                                            ...
                                            <button type="button" title="Supprimer"  class="btn btn-danger btn-circle btn-xs  margin-bottom-5 waves-effect waves-light"><a href="{{ url('delete-utilisateur-') }}{{$user->idUser}}" style="color:white;"><i class="material-icons">delete_sweep</i></a> </button>
                                            @endif

                                            @if(in_array("reset_user", session("auto_action")))
                                            <button type="button" title="Réinitialiser"  class="btn btn-warning btn-circle btn-xs  margin-bottom-5 waves-effect waves-light"><a href="{{ url('reinitialiser-utilisateur-') }}{{$user->idUser}}" style="color:white;"> <i class="material-icons">restore</i></a></button>
                                            @endif

                                            @if(in_array("status_user", session("auto_action")))
                                                @if($user->statut == "0")
                                                <button type="button" title="Désactivé ?"  class="btn btn-primary btn-circle btn-xs  margin-bottom-5 waves-effect waves-light"><a href=" {{ url('desactivé-utilisateur-') }}{{$user->idUser}}" style="color:white;"> <i class="material-icons">toggle_on</i></a></button>
                                                @endif
                                                @if($user->statut == "1")
                                                <button type="button" title="Activé ?"  class="btn btn-primary btn-circle btn-xs  margin-bottom-5 waves-effect waves-light" style="background-color:grey"><a href="{{ url('activé-utilisateur-') }}{{$user->idUser}}" style="color:white;"> <i class="material-icons">toggle_off</i></a></button>
                                                @endif
                                            @endif

                                            @if(in_array("status_user", session("auto_action")))
                                                @if($user->activereceiveincident == 0)
                                                <button type="button" title="Voulez-vous désactivé l'utilisateur à recevoir les mails d'incident ?"  class="btn btn-primary btn-circle btn-xs  margin-bottom-5 waves-effect waves-light"><a href=" {{ route('DSUM', $user->idUser) }}" style="color:white;"> <i class="material-icons">contact_mail</i></a></button>
                                                @endif
                                                @if($user->activereceiveincident == 1)
                                                <button type="button" title="Voulez-vous activé l'utilisateur à recevoir les mails d'incident ?"  class="btn btn-primary btn-circle btn-xs  margin-bottom-5 waves-effect waves-light" style="background-color:grey"><a href="{{ route('ATUM', $user->idUser)}}" style="color:white;"> <i class="material-icons">mail_outline</i></a></button>
                                                @endif
                                            @endif
