...
class RoleController extends Controller
{
            ...
            if (!in_array("menu_role", session("auto_action"))) {
                ...
                for ($i = 0; $i < count($array_men); $i++) {
                    if (!in_array(
                        $array_men[$i],
                        $menu
                    ) && isset(DB::table('action_menu_acces')->where('Menu', $array_men[$i])->first()->id)) {
                ...
                for (
                    ...
                    $i < count($addAction);
                    $i++
                ) { // Parcourir les actions sélectionnés 
                ...
                for ($i = 0; $i < count($array_act); $i++) {
                    if (!in_array(
                        $array_act[$i],
                        $addAction
                    ) && isset(DB::table('action_menu_acces')->where('Role', $role)->where('Menu', DB::table('action_menus')->where('id', $array_act[$i])->first()->Menu)->where('ActionMenu', $array_act[$i])->first()->id)) {

                        DB::table('action_menu_acces')->where('ActionMenu', $array_act[$i])->where('Role', $role)->update(["statut" => 1]);
                    }
                }

                flash('Les menus sont bien attribués au rôle')->success();
                return Back();
            }
    ...
    public static function ExisteRole($libelle)
    {
        ...
        if (isset($role) && $role->idRole != 0) return true;
        else return false;
    }
